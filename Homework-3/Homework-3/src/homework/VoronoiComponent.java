...
public class VoronoiComponent extends JComponent {
    ...
    public void paintComponent(Graphics g) {
        ...
        if (seedCount > 0) {
            ...
            //------------------------ paint the seeds -----------------------//
            if (showSeeds) {
                for (Seed seedList1 : seedList) {
                    Point p = seedList1.getPoint();
                    double x = p.x;
                    double y = p.y;
                    if (distanceVisualization) {
                        g2.setColor(Color.WHITE);
                    } else if (seedList1 == cursorSeed) {
                        g2.setColor(Color.LIGHT_GRAY);
                    } else {
                        g2.setColor(Color.invertColor(seedList1.getColor()));
                    }
                    g2.fill(seedList1.getCircle());
                }
            }
        }
    }
    ...
    private void updateMinMax() {
        for (Seed s1 : seedList) {
            for (Seed s2 : seedList) {
                double dist = getEuclidianDistance(s1.getPoint(), s2.getPoint());
                if (dist != 0) {
